...
static IS_RUNNUNG: AtomicBool = AtomicBool::new(false);
...
impl Client
{
    ...
    where F:  Send + Sync + Copy + 'static + Fn(WebsocketMessage)
    {
        ...
        {
            while Self::start(addr.clone(), f, 0, 15).await == false
            {
    ...
    where F:  Send + Copy + 'static + Fn(WebsocketMessage)
    {
        ...
        IS_RUNNUNG.store(true, std::sync::atomic::Ordering::SeqCst);
        ...
        IS_RUNNUNG.store(false, std::sync::atomic::Ordering::SeqCst);
    ...
    {
        if !IS_RUNNUNG.load(std::sync::atomic::Ordering::SeqCst)
        {
